		...
		if(true){
			...
			else if(IF_EnableLatch.isIF_enable() == true)
			{
				if (IF_EnableLatch.isFreeze() == true){
					...
					Simulator.getEventQueue().addEvent(new MemoryReadEvent(Clock.getCurrentTime()+containingProcessor.getL1iCache().latency,this,containingProcessor.getL1iCache(),containingProcessor.getRegisterFile().getProgramCounter()));
					IF_OF_Latch.setOF_enable(false);
